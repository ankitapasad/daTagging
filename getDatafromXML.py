...
fileListDialAct = sorted(os.listdir(startDir+'xml/dialAct/'))
namespaceIdentifier = '{http://nite.sourceforge.net/}'
dataFileTrain = open(startDir+'dataset_train.txt',"w")
dataFileDev = open(startDir+'dataset_dev.txt',"w")
dataFileTest = open(startDir+'dataset_test.txt',"w")
...
def getDialAct(fileName):
	tree = ET.parse(startDir+'xml/dialAct/'+fileName)
	root = tree.getroot()
	dialActdict = {}
	...
	for child in root:
	    ...
	    for pointer in pointers_to_word:
	        ...
	        dialActdict[(daId, label)].append(wordId)

	return dialActdict

def getUtt(fileName):
	tree = ET.parse(startDir+'xml/terminals/'+fileName.replace('dialAct','terminals'))
	root = tree.getroot()
	wordDict = {}
...
noDevConv = 0
noTestConv = 0
for fileName in fileListDialAct:
	dialActDict = getDialAct(fileName)
	wordDict = getUtt(fileName)
	convId = fileName.split('.')[0]
	uttId = 0
	for keys in dialActDict: # each key corresponds to one utterance
		utt = []
		label = keys[1]
		for wordId in dialActDict[keys]:
			if(wordId in wordDict):
				if(len(utt)==0): startTime = wordDict[wordId][1]
				endTime = wordDict[wordId][2]
				utt.append(wordDict[wordId][0])
			# else:
				# print(fileName, wordId)
		utt = ' '.join(utt)
		dataPoint = convId+'_'+str(uttId)+'\t'+utt+'\t'+label+'\t'+startTime+'\t'+endTime+'\n'
		...
		if(convId in train_set_idx): 
			if(uttId==0): noTrainConv += 1
			dataFileTrain.write(dataPoint)
		elif(convId in valid_set_idx): 
			if(uttId==0): noDevConv += 1
			dataFileDev.write(dataPoint)
		elif(convId in test_set_idx): 
			if(uttId==0): noTestConv += 1
			dataFileTest.write(dataPoint)
...
dataFileTest.close()
 ...
 def splitFiles(label,fileNames):
 # saving filenames into a text file
 	file = open('dataSplit/daFiles/'+label+'Files.txt',"w")
 	for fileName in fileNames:
 		file.write(fileName+'\n')
 	file.close()

splitFiles('train',train_set_idx)
splitFiles('dev',valid_set_idx)
splitFiles('test',test_set_idx)


def readFile(label):
# reading filenames into a vector
	lines = open('dataSplit/asrFiles/'+label+'Files.txt').readlines()
	fileNames = []    
	for line in lines:
		line = line.strip('\n')
		if(label=='test'): line=line[:2]+'0'+line[3:]
		fileNames.append(line[:2]+line[3:])   

	return fileNames

asrTrain = readFile('train')
asrDev = readFile('dev')
asrTest = readFile('test')

def getStats1(fileNameList):
# get counts of overlap between data splits
	inTrain = 0
	inDev = 0
	inTest = 0
	other = 0
	for fileName in fileNameList:
		if(fileName in daTrain): inTrain += 1
		elif(fileName in daDev): inDev += 1
		elif(fileName in daTest): inTest += 1
		else: other += 1

	print(inTrain, inDev, inTest, other)

def getStats2(fileNameList):
# get counts of overlap between data splits
	inTrain = 0
	inDev = 0
	inTest = 0
	other = 0
	for fileName in fileNameList:
		if(fileName in asrTrain): inTrain += 1
		elif(fileName in asrDev): inDev += 1
		elif(fileName in asrTest): inTest += 1
		else: other += 1
