...
def labelToId(truncDAs): # all distinct da in trunckDAs
	idDict = {}
	idNo = 0
...
def getDialAct(fileName,namespaceIdentifier,dataDir,dialActDict=None,uttDict=None,sideDict=None,truncDAs=None):
	tree = ET.parse(dataDir+'dialAct/'+fileName)
	root = tree.getroot()
	if(not dialActDict): dialActDict = {}
	if(not uttDict): uttDict = {}
...
def getUtt(fileName,namespaceIdentifier,dataDir,wordDict=None):
	tree = ET.parse(dataDir+'terminals/'+fileName.replace('dialAct','terminals'))
	root = tree.getroot()
	if(not wordDict): wordDict = {}

	for child in root:
		startTime = child.get(namespaceIdentifier+'start')
		endTime = child.get(namespaceIdentifier+'end')
		word = child.get('orth')
		wordId = child.get(namespaceIdentifier+'id')
		wordDict[wordId] = (word,startTime,endTime)
...
if __name__=="__main__":
	...
	startDir = '/share/data/speech/ankitap/daTagging/dataset/'
	fileListDialAct = sorted(os.listdir(dataDir+'dialAct/'))
	namespaceIdentifier = '{http://nite.sourceforge.net/}'
	dataFileTrain = open(startDir+format+'/dataset_train.txt',"w")
	dataFileDev = open(startDir+format+'/dataset_dev.txt',"w")
	dataFileTest = open(startDir+format+'/dataset_test.txt',"w")
	## one-time operation: saving the list of files
	# trainFileList = open(startDir+'daFiles/trainFiles.txt',"w")
	# devFileList = open(startDir+'daFiles/devFiles.txt',"w")
	# testFileList = open(startDir+'daFiles/testFiles.txt',"w")
	...
	noTestConv = 0
	filesUsed = []
	for fileNo in range(len(fileListDialAct)):
		fileName1 = fileListDialAct[fileNo]
		convId = fileName1.split('.')[0]
		if((convId in train_set_idx) or (convId in valid_set_idx) or (convId in test_set_idx)):
			if(fileName1 not in filesUsed):
				# if(convId in train_set_idx): trainFileList.write(convId+'\n')
				# elif(convId in valid_set_idx): devFileList.write(convId+'\n')
				# else: testFileList.write(convId+'\n')
				filesUsed.append(fileName1)
				fileName2 = fileName1.replace('.A','.B') if ('.A' in fileName1) else fileName1.replace('.B','.A')
				# getFileName(fileName1)
				filesUsed.append(fileName2)
				# updated with utterances from both sides
				dialActDict, uttDict, sideDict = getDialAct(fileName1,namespaceIdentifier,dataDir)
				dialActDict, uttDict, sideDict = getDialAct(fileName2,namespaceIdentifier,dataDir,dialActDict,uttDict,sideDict)
				wordDict = getUtt(fileName1,namespaceIdentifier,dataDir)
				wordDict = getUtt(fileName2,namespaceIdentifier,dataDir,wordDict)
				
				keys = list(range(1,len(uttDict)+1))
				if(format=='shuffled'): shuffle(keys)
			 
				# for key in sorted(uttDict.keys()):
				for key in keys: 
					convSide = fileName1.split('.')[0]+'_'+sideDict[key] # fileName_side
					utt = []
					...
					utt = ' '.join(utt)
					dataPoint = convSide+'_'+str(key)+'\t'+utt+'\t'+label+'\t'+startTime+'\t'+endTime+'\n'
					# if(int(convId[2:])<4001): 
					# 	if(uttId==0): noTrainConv += 1
					# 	dataFileTrain.write(dataPoint)
					# elif(int(convId[2:])<4154): 
					# 	if(uttId==0): noDevConv += 1
					# 	dataFileDev.write(dataPoint)
					# else: 
					# 	if(uttId==0): noTestConv += 1
					# 	dataFileTest.write(dataPoint)

					if(convId in train_set_idx): dataFileTrain.write(dataPoint)
					elif(convId in valid_set_idx): dataFileDev.write(dataPoint)
					else: dataFileTest.write(dataPoint)
